...
const char* LINE_USER_ID = "自己的";
const unsigned long LINE_INTERVAL = 30000;
...
void drawScreen() {
  ...
  if (uploadStatus == "LINE_NOTIFY") {
    oled.setFont(u8g2_font_wqy12_t_chinese1);
    oled.drawUTF8(2, 43, "LINE");
    oled.drawUTF8(2, 54, "正在傳訊息通知");
  } else {
...
bool sendLineNotification() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("LINE notification skipped: Wi-Fi disconnected");
    uploadStatus = "LINE ERR";
    drawScreen();
    return false;
  }

  String message = "異常通知\n溫度: " + String(temperature) + " C\n濕度: " + String(humidity) + " %";
  String payload = "{\"to\":\"" + String(LINE_USER_ID) +
  ...
  uploadStatus = "LINE_NOTIFY";
  drawScreen();
  Serial.println("LINE正在傳訊息通知");
  ...
  lineClient.println(String("Content-Length: ") + payload.length());
  lineClient.println();
  lineClient.println(payload);

  String statusLine = lineClient.readStringUntil('\n');
  bool success = statusLine.startsWith("HTTP/1.1 200") || statusLine.startsWith("HTTP/1.0 200");
  String responseBody;
  unsigned long responseTimeout = millis();
  while (lineClient.connected() && millis() - responseTimeout < 5000) {
    while (lineClient.available()) {
      responseBody += lineClient.readString();
      responseTimeout = millis();
    }
  }
  lineClient.stop();
  uploadStatus = success ? "LINE OK" : "LINE ERR";
  drawScreen();
...
void loop() {
  readSensors();
  bool abnormal = dhtError == SimpleDHTErrSuccess && (temperature > 28 || humidity > 70);
  drawScreen();

  if (abnormal && (!wasAbnormal || millis() - lastLineNotify >= LINE_INTERVAL)) {
